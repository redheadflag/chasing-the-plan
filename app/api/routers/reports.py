...
def _filename(workouts: list[Workout], ext: str) -> str:
    """Filename without the athlete's name; single-workout downloads use its title."""
    if len(workouts) == 1:
        w = workouts[0]
...
def download_plan_xlsx(
    ...
    doc, workouts = _load(db, athlete_id, days, workout_id)
    content = render_xlsx(doc)
    return Response(
        content=content,
        media_type=XLSX_MIME,
        headers={"Content-Disposition": _content_disposition(_filename(workouts, "xlsx"))},
    )
...
def download_plan_pdf(
    ...
    return Response(
        content=content,
        media_type=PDF_MIME,
        headers={"Content-Disposition": _content_disposition(_filename(workouts, "pdf"))},
    )
